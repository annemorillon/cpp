...
int main()
{
    ...
    std::cout << YELLOW "\n--- Test 4: Copy & Assignment ---" RESET << std::endl;
    {
        ...
        std::cout << "Copy to std::stack size: " << s.size() << (*const_stack.begin() == 5 ? GREEN " [OK]" : RED " [KO (Expected: 5)]") << RESET << std::endl;

        MutantStack<int> assigned_stack;
        assigned_stack = mstack;
        std::cout << "Assigned stack top: " << assigned_stack.top() << " (Expected: 0)" << std::endl;
